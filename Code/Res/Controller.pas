...
If GameMode=0 then begin
  ...
  begin
     StarButton.BShowing:=false;
      ...
      minmap.Create(Ttextobj[27],MGEW/8,MGEH-MGEH/8,MGEW/10,MGEH/10);
      minmap.Draw(-Phantom.p.x+880,Phantom.p.z-110,5250,Phantom.AngleX+90,0.07+(abs(Phantom.Speed/8)));
  ...
  RenderSprite(MGEW-MGEW/10,MGEH-MGEW/11.5,MGEW/10,MGEW/10,-130+(abs(Phantom.ARMS)*10000),TGATex[4]);
   ...
   {  AddO.BShowing:=true;
     AddO.PutBut((MGEH*0.1),(MGEH*0.1),MGEW*0.5,(MGEH*0.4));
     AddO.Aktivate(GUICursor);

     SaveO.BShowing:=true;
     SaveO.PutBut((MGEH*0.1),(MGEH*0.1),MGEW*0.5,(MGEH*0.6));
     SaveO.Aktivate(GUICursor);
                    }
...
{    3d сцена     }
   ...
   {  if (AddO.State=2) then begin
       SetLength(MassP,Length(MassP)+1);
       MassP[Length(MassP)-1].x:=view.p.x;
       MassP[Length(MassP)-1].y:=view.p.y;
       MassP[Length(MassP)-1].z:=view.p.z;
    end;

     if (SaveO.State=2) then begin
        Assignfile(fi,'Testmap.txt'); rewrite(fi);
        for I := 0 to Length(MassP)-1 do begin
          writeln(fi, 'SetP3D('+IntTostr(round(MassP[i].x))+','
                                +IntTostr(round(MassP[i].y))+','
                                +IntTostr(round(MassP[i].z))+'),');
        end;
        closefile(fi);
    end;     }
