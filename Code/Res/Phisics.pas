...
var I,N,L,K,PP,PS,PF,PL,TWF:integer;
    ...
    TempAngle1,TempAngle2,D1,D2,TS, KRot:real;
    TempVec,TempVec1,TempVec2,TempVecX,TempVecY:MGE3DVec;
begin
...
if FizCheched>0 then  begin
  ...
  KRot:=1;
  if abs(MCP[0].Speed)>0 then KRot:=abs(1-(MCP[0].Speed/(MSpeed*1.5)));

  if (KD<>false)  then WeelRot:=WeelRot-0.15*KRot;
  if (KA<>false)  then WeelRot:=WeelRot+0.15*KRot;
